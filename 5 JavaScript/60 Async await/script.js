...
//     .then((details)=> console.log('details:-',details))to

//In async we keep everything as it is, just change op code
const disply = async()=> {
    const usr = await fetches('suraj');
    const photo = await fetchPhoto(usr.username);
    const details = await fetchDetails(photo[0]);
    console.log(details); 
}
disply();